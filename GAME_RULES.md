...
- A rolled die remains on the table while its player chooses a move and disappears immediately after that move is recorded: “pickup your trash.” If a roll has no legal move, the die disappears one second after landing while the status card briefly says `[Player] rolled a [N], cannot move.`
- Polling must never skip a real player's throw. If a viewer receives a roll and its completed move together, the viewer still sees the full accurate throw before the resolved die is cleared. A remote no-move throw completes its animation, displays the landed result for one second, and keeps the result sentence visible briefly afterward.
