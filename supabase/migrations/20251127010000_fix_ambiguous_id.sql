-- Fix: Ambiguous column reference 'id' in move_marble function
-- Created: 2025-11-27
-- Purpose: Fix ambiguous id reference in capture check query
...
BEGIN
  ...
  -- FIX: Use m.id instead of ambiguous id
  IF v_new_position_type = 'track' THEN
    SELECT m.id INTO v_captured_marble_id
    FROM marbles m
